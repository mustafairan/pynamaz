...
class PyNamaz(QtGui.QMainWindow, Ui_MainWindow):
    ...
    warningTime = "00:30:00"  # indicates the warning time before next time becomes !!no use anymore!!
    ...
    homedir = expanduser('~')  # coz  python doesnt understand tilda. this is the prefix location for main folder. default tilda is for linux. %ProgramFiles%\ (\pyNamaz) in windows
    ...
    def openPrayerTimesText(self):
        ...
        if self.platform == "linux":
            ...
            system(
                'start notepad "' + self.homedir + r'\data\PrayerTimes.txt"')  # TODO this usage can cause problem. file paths should be specified in more efficient way. should be tested in windows systems
        else:
            pass
